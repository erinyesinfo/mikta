...
class News extends Component {
    async componentDidMount() {
        if (this.props.HackerNews.length === 0) {
            ...
            results.data.slice(r, r + 10).map(async element => {
                ...
                this.props.handleHackerNewsApi(innerResults.data);
            });
        }
    };
    handleRefreshNews = async () => {
        ...
        results.data.slice(r, r + 10).map(async element => {
            ...
            this.props.handleHackerNewsApi(innerResults.data, true);
        });
    };
    handleMoreNews = async () => {
        ...
        results.data.slice(r, r + 10).map(async element => {
            ...
            this.props.handleHackerNewsApi(innerResults.data, false);
        });
    };
    render() {
        const HackerNews = this.props.HackerNews.map(news => {
            return (
                <div className='news-div' key={news.id}>
                    ...
                    <a className='news-user' rel="noopener noreferrer" href={news.url} target='_blank'>{news.title}</a>
                </div>
            );
        });
